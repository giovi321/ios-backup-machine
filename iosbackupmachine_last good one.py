...
class Panel:
    def __init__(self):
        ...
        # Physical buffer size reported by driver (portrait native)
        self.pw, self.ph = self.epd.width, self.epd.height  # likely 250 x 122
        self.anim = 0
    ...
    def draw(self, subtitle, percent=None, animate=True, center_block=None, show_tail_lines=None, show_header=True):
        ...
        if show_header:
            ...
            if percent is None:
                # center subtitle as a block under the header
                ...
                px = x + (w - ptw) // 2
                py = y + (h - pth) // 2
                drw.rectangle((px - 2, py - 1, px + ptw + 2, py + pth + 1), fill=255)
                drw.text((px, py), pct, font=F_MD, fill=0)

                # then center the subtitle below the bar
                lines = [p for p in subtitle.split("\n") if p.strip()]
                y2 = y + h + 6  # 6px gap below bar
                for ln in lines:
        ...
        if percent is not None and show_header:
            x, y, w, h = 4, 46, LW - 8, 18
            drw.rectangle((x, y, x+w, y+h), outline=0, width=2)
            fill_w = int(max(0, min(100, percent)) * (w - 4) / 100)
            drw.rectangle((x+2, y+2, x+2+fill_w, y+h-2), fill=0)
        ...
        # rotate logical canvas to physical buffer orientation
        out = self._rotate_for_display(img)
        # ensure final size matches physical buffer
...
#        panel.draw("", percent=None, animate=False, center_block=center, show_header=False)
    ...
    def feed_parser(tok: str):
        nonlocal cur_line, pct, encrypted, last_ui
